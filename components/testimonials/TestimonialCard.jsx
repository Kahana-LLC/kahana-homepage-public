...
export default function TestimonialCard({
  ...
  return (
    <article
      ...
      className={`scroll-mt-28 rounded-2xl border border-[#3B2F1A]/12 bg-white/90 p-6 shadow-[0_10px_40px_rgba(59,47,26,0.06)] backdrop-blur-sm sm:p-8 ${
        isFeatured
          ? "sm:text-[1.02rem] lg:shadow-[0_14px_50px_rgba(59,47,26,0.09)]"
          : ""
      } ${!isPreview ? "transition-shadow duration-300 hover:shadow-[0_16px_48px_rgba(59,47,26,0.1)]" : "h-full flex flex-col"}`}
    >
      <QuoteMark className="h-8 w-8 text-[#8A6622]/35" />
      <blockquote className="mt-4 flex-1">
        {body.map((paragraph, index) => (
          <p
            key={index}
            className={`leading-relaxed text-[#3B2F1A]/82 ${
              index > 0 ? "mt-4" : ""
      ...
      <footer className="mt-6 flex items-center gap-3 border-t border-[#3B2F1A]/10 pt-5">
        {avatarSrc ? (
          <div className="relative h-11 w-11 shrink-0 overflow-hidden rounded-full border border-[#3B2F1A]/12 bg-[#EDE6D2]">
            <Image
              src={avatarSrc}
              alt=""
              width={AVATAR_SIZE}
          ...
          <div
            className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#8A6622]/15 text-sm font-bold text-[#8A6622]"
            aria-hidden
          >
            {getInitials(testimonial.name)}
          </div>
        )}
        <cite className="not-italic text-base font-semibold text-[#3B2F1A]">
          {testimonial.name}
        </cite>
      </footer>
    </article>
