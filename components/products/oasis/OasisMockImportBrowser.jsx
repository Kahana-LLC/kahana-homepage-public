...
export function OasisMockImportBrowser() {
  ...
  return (
    <div
      className="relative min-h-[300px] overflow-hidden rounded-2xl border border-[#4A5745]/12 bg-[#e8eae4] shadow-md sm:min-h-[340px]"
    ...
    >
      <div className="p-4 sm:p-5">
        <p className="text-[10px] font-semibold uppercase tracking-wide text-[#4A5745]/55">General</p>
        <h3 className="mt-1 text-sm font-bold text-[#2f3a20]">Import browser data</h3>
        <p className="mt-1 max-w-md text-[11px] leading-relaxed text-[#4A5745]/85">
          Bring your bookmarks, passwords, history, extensions, and autofill data from another browser.
        </p>
        <div className="mt-3 inline-block rounded-lg border border-[#4A5745]/12 bg-[#dfe2db] px-4 py-2 text-center text-[11px] font-semibold text-[#4A5745]">
          Import data
        </div>
      </div>
      ...
      <div className="absolute inset-0 flex items-center justify-center bg-black/[0.04] px-3 py-6 sm:px-6">
        <motion.div
          className="w-full max-w-[300px] rounded-xl border border-[#4A5745]/10 bg-white p-4 shadow-xl"
            ...
            {BROWSER_ROWS.map((row) => (
              <li
                ...
                className={`flex items-center gap-2 rounded-lg px-2 py-1.5 ${
                  row.selected ? 'bg-[#4A5745]/10' : 'hover:bg-black/[0.03]'
                }`}
              >
                <BrowserGlyph variant={row.icon} />
                <span className={`truncate ${row.selected ? 'font-semibold text-[#2f3a20]' : 'text-[#4A5745]'}`}>
                  {row.label}
                </span>
              </li>
            ))}
            <li className="mt-2 border-t border-black/5 pt-2 text-[#4A5745]">
