      ...
      <div className="product-container relative">
        <div className="flex flex-col items-center justify-center text-center">
          ...
          <div className="w-full max-w-4xl mx-auto px-4 mb-12">
            <h2 className="text-xl font-semibold leading-8 text-[#617500] mb-4">Walk into Oasis</h2>
            <p className="text-2xl font-semibold tracking-tight text-[#333333] sm:text-3xl mb-8">
              First AI browser designed for ergonomic work, focus, and spatial ease
            </p>
          ...
          <div className="w-full max-w-6xl mx-auto px-4 relative mt-12">
            <div className="w-full mx-auto aspect-[24/9] lg:aspect-[16/9] overflow-hidden rounded-2xl shadow-2xl border border-[#30400D]/10 group hover:border-[#30400D]/20 transition-all duration-300" style={{ background: 'linear-gradient(90deg, #F8FAF2 0%, #d6e3f4 100%)' }}>
              <div className="relative h-full w-full">
                {/* Browser Header */}
                <div className="flex h-8 sm:h-12 items-center justify-between px-2 sm:px-4" style={{ background: 'linear-gradient(90deg, #F8FAF2 0%, #d6e3f4 100%)' }}>
                ...
                {/* Browser Content */}
                <div className="relative h-[calc(100%-2rem)] sm:h-[calc(100%-3rem)]" style={{ background: 'linear-gradient(90deg, #F8FAF2 0%, #d6e3f4 100%)' }}>
                  {/* URL Bar */}
                  ...
                  <div className="relative flex-1 p-2 sm:p-4">
                    <div className="text-center mb-4 sm:mb-6 flex items-center justify-center space-x-1.5 sm:space-x-2">
                      <svg className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-[#313A00]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
                      </svg>
                      <span className="text-[10px] sm:text-xs font-light tracking-wide text-[#313A00]">Unicorn Inc.</span>
                    </div>
                    {/* Shortcuts Grid */}
                    <div className="grid grid-cols-3 sm:grid-cols-5 gap-x-8 gap-y-4 sm:gap-4 max-w-[280px] sm:max-w-lg mx-auto px-4 sm:px-0">
                      {[
                        { 
                          icon: <svg className="w-6 h-6 text-[#313A00]" fill="currentColor" viewBox="0 0 24 24"><path d="M19.615 3.184c-3.604-.246-11.631-.245-15.23 0-3.897.266-4.356 2.62-4.385 8.816.029 6.185.484 8.549 4.385 8.816 3.6.245 11.626.246 15.23 0 3.897-.266 4.356-2.62 4.385-8.816-.029-6.185-.484-8.549-4.385-8.816zm-10.615 12.816v-8l8 4-8 4z"/></svg>,
                          label: "YouTube",
                          showOnMobile: true
                        },
                        {
                          icon: <svg className="w-6 h-6 text-[#313A00]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>,
                          label: "Inbox (7)",
                          showOnMobile: true
                        },
                        {
                          icon: <svg className="w-6 h-6 text-[#313A00]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>,
                          label: "Dashboard",
                          showOnMobile: true
                        },
                        {
                          icon: <svg className="w-6 h-6 text-[#313A00]" fill="currentColor" viewBox="0 0 24 24"><path d="M19 3a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h14m-.5 15.5v-5.3a3.26 3.26 0 0 0-3.26-3.26c-.85 0-1.84.52-2.32 1.3v-1.11h-2.79v8.37h2.79v-4.93c0-.77.62-1.4 1.39-1.4a1.4 1.4 0 0 1 1.4 1.4v4.93h2.79M6.88 8.56a1.68 1.68 0 0 0 1.68-1.68c0-.93-.75-1.69-1.68-1.69a1.69 1.69 0 0 0-1.69 1.69c0 .93.76 1.68 1.69 1.68m1.39 9.94v-8.37H5.5v8.37h2.77z"/></svg>,
                          label: "LinkedIn",
                          showOnMobile: false
                        },
                        {
                        ...
                        {
                          icon: <svg className="w-6 h-6 text-[#313A00]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>,
                          label: "Calendar",
                          showOnMobile: false
                        },
                        {
                          icon: <svg className="w-6 h-6 text-[#313A00]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" /></svg>,
                          label: "Drive",
                          showOnMobile: false
                        },
                        {
                          icon: <svg className="w-6 h-6 text-[#313A00]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>,
                          label: "Add shortcut",
                          showOnMobile: false
                        }
                      ].map((shortcut, index) => (
                        ...
                        >
                          ...
                          <span className="mt-2 text-[10px] sm:text-xs text-[#313A00] w-full text-center truncate px-1">{shortcut.label}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="absolute bottom-4 right-4 flex items-end">
                    ...
                    {isChatOpen && (
                      <div className="hidden lg:flex flex-col mr-3 w-80 h-72 bg-white rounded-2xl overflow-hidden">
                        <div className="flex items-center justify-between p-4 border-b border-[#AFBE66]" style={{ background: 'linear-gradient(90deg, #F8FAF2 0%, #d6e3f4 100%)' }}>
                          <div className="flex items-center space-x-3">
                            <div className="w-8 h-8 rounded-lg bg-[#7A9200] border border-[#AFBE66] flex items-center justify-center">
                              <svg className="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
                              </svg>
                            </div>
                            <span className="font-medium text-[#7A9200]">AI Assistant</span>
                          </div>
                          <button onClick={() => setIsChatOpen(false)} className="text-[#7A9200] hover:text-[#6A8200]">
                            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                            </svg>
                          </button>
                        </div>
                        <div className="flex-1 p-4" style={{ background: 'linear-gradient(90deg, #F8FAF2 0%, #d6e3f4 100%)' }}>
                          <div className="flex items-start space-x-3 mb-4">
                            <div className="w-6 h-6 rounded-lg bg-[#7A9200]/10 flex-shrink-0 flex items-center justify-center">
                              <svg className="w-4 h-4 text-[#7A9200]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
                              </svg>
                            </div>
                            <div className="bg-white rounded-2xl">
                              <p className="text-sm text-[#7A9200]">
                                {showDemoMessage && (
                                  <>Unfortunately, this little chat bot on our landing page isn't integrated with AI. To see the real thing, <Link href="/schedule-demo" className="text-[#7A9200] hover:underline">schedule a demo</Link>! 😉</>
                                )}
                              </p>
                            </div>
                          </div>
                        </div>
                        <div className="p-4 bg-white border-t border-[#AFBE66]">
                          <form onSubmit={handleSendMessage} className="flex space-x-2">
                            <input
                              ...
                              className="flex-1 px-4 py-2 text-sm rounded-xl focus:outline-none focus:ring-2 focus:ring-[#7A9200]/20 focus:border-[#7A9200]"
                            />
                            <button 
                              type="submit"
                              className="p-2 text-white font-bold bg-[#7A9200] border border-[#AFBE66] rounded-xl hover:bg-[#6A8200] hover:border-[#9FAE56] transition-all duration-300"
                            >
                              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <button
                      ...
                      className="btn-primary group flex items-center space-x-2 px-3 py-2 cursor-pointer"
                    >
